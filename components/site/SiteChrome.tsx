...
function Wordmark({ size = 'md' }: { size?: 'md' | 'sm' }) {
  ...
  return (
    <span className="flex items-center gap-2.5">
      <span
        className={`grid ${box} place-items-center rounded-xl font-extrabold`}
...
export default function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh">
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:m-3 focus:rounded-lg focus:px-4 focus:py-2 focus:text-sm focus:font-bold"
        style={{ background: 'var(--brand-solid)', color: 'var(--brand-ink)' }}
      >
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--line-soft)] bg-[var(--bg)]/85 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center gap-3 px-4 sm:px-6">
          ...
          <div className="ms-auto flex items-center gap-1.5">
            ...
            <Link
              href="/habits"
              className="ms-1 hidden rounded-lg px-4 py-2 text-sm font-bold transition-opacity hover:opacity-90 sm:inline-block"
              style={{ background: 'var(--brand-solid)', color: 'var(--brand-ink)' }}
            >
              {c.nav.open}
            </Link>
      ...
      <footer className="border-t border-[var(--line-soft)] bg-[var(--bg-2)]">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <Wordmark size="sm" />
              <p className="mt-2 max-w-xs text-xs leading-relaxed text-[var(--fg-faint)]">
                {c.footer.tagline}
              </p>
            </div>
